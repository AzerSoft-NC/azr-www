...
The picker lives in the header as a pill-shaped dropdown right next to the colour-theme pill. It shows the icon for the _currently chosen mode_ (monitor, sun, or moon), so the user can always see what they picked at a glance. Below the `md` breakpoint both pills hide from the header bar and reappear inside the mobile menu — selecting a mode in one updates the other instantly.
...
- **Patterns** — ContactForm, SearchInput, StatCard, FormField, EmptyState, LetterGlitchBand
